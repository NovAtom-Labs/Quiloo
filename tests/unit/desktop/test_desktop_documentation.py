...
def test_readme_leads_with_native_alpha_downloads() -> None:
    ...
    for phrase in (
        ...
        "DEVSIM",
        "Sentaurus",
        "v0.1.0-alpha.9/Agent-Kronig-0.1.0-alpha.9-linux-x64.AppImage",
        "v0.1.0-alpha.9/Agent-Kronig-0.1.0-alpha.9-win-x64.exe",
        "v0.1.0-alpha.9/Agent-Kronig-0.1.0-alpha.9-mac-x64.dmg",
        "v0.1.0-alpha.9/Agent-Kronig-0.1.0-alpha.9-mac-arm64.dmg",
    ):
        assert phrase in readme
        if "v0.1.0-alpha.9/" in phrase:
...
def test_alpha_release_and_community_documents_are_complete() -> None:
    release_notes = (ROOT / "docs" / "releases" / "v0.1.0-alpha.9.md").read_text()
...
def test_new_public_documents_do_not_use_em_dashes() -> None:
    paths = (
        ...
        ROOT / "CONTRIBUTING.md",
        ROOT / "SECURITY.md",
        ROOT / "docs" / "releases" / "v0.1.0-alpha.9.md",
        ROOT / ".github" / "PULL_REQUEST_TEMPLATE.md",
        ROOT / ".github" / "ISSUE_TEMPLATE" / "bug-report.yml",
        ROOT / ".github" / "ISSUE_TEMPLATE" / "config.yml",
    )
