...
- The Agent pane separates Chat, Activity, and Changes. A compact header selector switches persistent conversations without occupying repository space. Chat keeps its composer anchored to the bottom, follows new messages when the reader is already near the newest entry, preserves position while older messages are being read, and shows active work in a narrow progress strip. Activity groups each tool start and completion into one expandable action with backend-recorded phase, duration, owner, command, output, affected files, and delegated-agent identity. A bounded current-operation summary is derived from structured tool events. Raw provider reasoning is neither stored nor displayed. Changes lists files changed during the selected run window and attaches action attribution only when it can be proven. Successful edit, terminal, and delegated actions receive bounded before-and-after comparisons. Validation evidence is linked only when the file hash recorded after a successful non-mutating check matches the terminal run manifest. Overlapping writers remain explicitly unattributed because the responsible action cannot be proven safely.
...
6. Use Activity for one row per agent action. Expand a row for its exact command or output. Current-operation summaries and technical events remain collapsed until requested. Raw provider reasoning is never stored or displayed.
